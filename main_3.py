...
#          # run get resources to get resource_id
        ...
        # create resource
         cmd_a = f'aws apigateway create-resource --rest-api-id {api_id} --parent-id {parent_id} --path-part {{proxy+}}'
         print(cmd_a)
...
#          # apply put-method to api
         cmd_4 = f'aws apigateway put-method --rest-api-id {api_id} --resource-id {resource_id} --http-method "ANY" --authorization-type "NONE"'
         print(cmd_4)
...
#          # give lambda permissions to be run by your api.
         cmd_8 = f'aws lambda add-permission --function-name {function_aws} --statement-id "{api_id}" --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:us-east-1:482102633168:{api_id}/*"'
         print(cmd_8)
